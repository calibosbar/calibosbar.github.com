...
fig = corner_plot(samples, labels = [r'$ t_{quench}[Gyrs]$', r'$ \tau [Gyrs]$'], extents=[[N.min(samples[:,0]), N.max(samples[:,0])],[N.min(samples[:,1]),N.max(samples[:,1])]], bf=[tq_mcmc, tau_mcmc], id=dr8)
