...
#cbar = plt.colorbar()
#cbar.ax.set_ylabel('Stellar Mass Density', rotation=270)
#img.shape
#img.min()
...
img_masked= img #*0.055555556
where_are_NaNs = isnan(img_masked)
img_masked[where_are_NaNs] = 0
#ticksy=np.linspace(0,2,8)
ticksx=np.arange(0,12)
...
cbar=plt.colorbar()
cbar.ax.set_ylabel('Surface Brightness $[L_{sun}/kpc^{2}]$', rotation=270, fontsize=8, verticalalignment='top')
plt.ylabel('r/re (pixels)')
plt.xlabel('Log (Age/yrs)')
plt.title('UGC11680NED01 Stellar Surface Brightness Intensity map') 

plt.figure()
x =np.linspace(0, 2, 36)
plt.ylabel('Log L(r) $[L_{sun}/kpc^{2}]$ ')
plt.xlabel('R/Re')
plt.yscale('log')
#plt.xscale('log')
