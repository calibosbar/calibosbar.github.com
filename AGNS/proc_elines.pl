...
#
$dev="4".$DEV;
($pdl_MAP_SMass_OH,$pdl_fMAP_SMass_OH)=plot_XY_color_scale($pdl_Sigma_Mass,$pdl_OH_O3N2,$pdl_log_EW_Ha,$pdl_ctable,$dev,-0.5,4.2,8.11,8.79,0,2.49,"log(\\gS\\dMass\\u/M\\d\\m9\\u pc\\u-2\\d)","12+log(O/H)","log10 |EW(H\\ga)|","$name");
...
$dev="5".$DEV;
plot_MAP_color_scale($pdl_MAP_SMass_OH,$pdl_fMAP_SMass_OH,$pdl_ctable,$dev,-0.5,4.2,8.11,8.79,0,2.49,"log(\\gS\\dMass\\u/M\\d\\m9\\u pc\\u-2\\d)","12+log(O/H)","log10 |EW(H\\ga)|","$name",1,1);
...
#	    print "$dist,$dist_min,$age_now,$age_mod[$iii],$ML_mod\n";
	    $dist_min=$dist;
	    $ML_now=$ML_mod;
	}
    }
    ...
    $t .= $tt*$ML_now*$L*(1e-16)/3.826e33;

#
# Run for metals!
#
    $pdl_Lum_coadd=zeroes($nx_now,$ny_now);
    for ($j=0;$j<4;$j++) {
	$II=$I*4+$j;
	my $t=$pdl_sfh_met->slice(":,:,($I)");
	my $Lum=$pdl_sfh->slice(":,:,($II)");
	$t .=$t+$met_mod[$II]*$Lum;#*$ml[$II];
	$pdl_Lum_coadd=$pdl_Lum_coadd+$Lum;
    }
    my $t=$pdl_sfh_met->slice(":,:,($I)");
    $t .= $t*$pdl_Lum_coadd;
...
$dev="11".$DEV;
($pdl_MAP_SMass_SFR,$pdl_fMAP_SMass_SFR)=plot_XY_color_scale($pdl_Sigma_Mass,$pdl_Sigma_SFR,$pdl_map_y_200,$pdl_ctable,$dev,-0.5,4.2,-10.5,-5.5,0,49,"log(\\gS\\dMass\\u/M\\d\\m9\\u pc\\u-2\\d)","log(\\gS\\dSFR\\u/M\\d\\m9\\u yr\\u-1\\d pc\\u-2\\d)","% Lum. Age<200 Myr","$name");
...
$dev="12".$DEV;
plot_MAP_color_scale($pdl_MAP_SMass_SFR,$pdl_fMAP_SMass_SFR,$pdl_ctable,$dev,-0.5,4.2,-10.5,-5.5,0,49,"log(\\gS\\dMass\\u/M\\d\\m9\\u pc\\u-2\\d)","log(\\gS\\dSFR\\u/M\\d\\m9\\u yr\\u-1\\d pc\\u-2\\d)","% Lum. Age<200 Myr","$name",1,1);


$dev="13".$DEV;
($pdl_MAP_SMass_SFR,$pdl_fMAP_SMass_SFR)=plot_XY_color_scale($pdl_Sigma_Mass,$pdl_Sigma_SFR,$pdl_log_EW_Ha,$pdl_ctable,$dev,-0.5,4.2,-10.5,-5.5,0,2.5,"log(\\gS\\dMass\\u/M\\d\\m9\\u pc\\u-2\\d)","log(\\gS\\dSFR\\u/M\\d\\m9\\u yr\\u-1\\d pc\\u-2\\d)","log |EW(H\\ga)|","$name");
...
$$h{CRPIX2}=1;
$pdl_map->sethdr($h);
plot_MAP_color_scale($V_img,$pdl_map,$pdl_ctable,$dev,0,0,0,0,-0.7,0.2,"RA (arcsec)","DEC (arcsec)","[Z/H]","$name",1,1,1);
...
$dev="43".$DEV;
($pdl_MAP_SMass_Av_gas,$pdl_fMAP_SMass_Av_gas)=plot_XY_color_scale($pdl_Av_org,$pdl_Sigma_Mass,$pdl_r,$pdl_ctable,$dev,,0,2.6,-0.5,4.2,0,3.1,"Av gas (mag)","log(\\gS\\dMass\\u/M\\d\\m9\\u pc\\u-2\\d)","R/Re","$name",0,0,1,1,$pdl_x_line,$pdl_y_line,1);
...
#$pdl_smooth->wfits("pdl.fits");
#$pdl_smooth=$pdl_smooth-log10(($DA)**2)-6; # Msun/pc^2
plot_MAP_color_scale_table($pdl_smooth,$pdl_smooth,$pdl_ctable,$dev,-2.5,0.2,"log(age/yr)","R/Re","[Z/H]","$name",0,1,$pdl_label_X,$pdl_label_Y);

$outfile=$name.".p_e.rad_SFH_Met.fits";
$pdl_smooth->wfits($outfile);

$dev="45_rad".$DEV;
plot_MAP_line_color_scale_table($pdl_smooth,$pdl_label_Y,$pdl_ctable,$dev,0,2.5,"log(age/yr)","[Z/H]","R/Re","$name",$pdl_label_X,$pdl_label_Y,-2.5,0.2);
...
$pdl_smooth->sethdr($h);
$dev="46".$DEV;
plot_CUBE_color_scale($pdl_sfh_lum_age,$pdl_smooth,$pdl_ctable,$dev,0,0,0,0,-2.5,0.2,"RA (arcsec)","DEC (arcsec)","[Z/H] ($name)","",0,1,1,8,5);
...
print OUT "$name,$log_Mass,$lSFR,$OH_O3N2,$a_ion_cen[3],$frac_area_GAS,$frac_area_SFR_pure,$Av_w\n";
close(OUT);

#$frac_area_SFR_pure=$subm_SFR_pure/(6*($nx/2)*sqrt(($nx/2)**2-($nx/4)**2));
#$frac_area_GAS=$sum_GAS/(6*($nx/2)*sqrt(($nx/2)**2-($nx/4)**2));
...
sub plot_MAP_line_color_scale_table {
 ...
 my $Y_MIN=$_[12];
    ...
    my $x_min=$stats_X[3]->at(0);
    my $x_max=$stats_X[4]->at(0);
    my $y_min,$y_max;
if (($Y_MIN==0)&&($Y_MAX==0)) {
